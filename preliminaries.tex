...
The cobar construction $\cobar C$ of this coaugmented dg coalgebra is the graded module
\begin{equation*}
...
We now review the central examples of coaugmented dg coalgebras on which Adams used this construction.
...
If $X \in \sSet^0$ is a reduced simplicial set with $X_0=\{b\}$ then $\schainsAS(X)$ becomes a connected dg coalgebra with the coaugmentation $\nu \colon \k \to \schains(X)$ induced by $\nu(1)=b \in\schains_0(X).$ Hence, the functor $\schainsAS$ restricts to a functor
...
Define the \textit{triangulation} functor $\mathcal{T} \colon \cSet \to \sSet$ by $$\mathcal{T}(C) = \colim_{\cube^n \to C} (\Delta^1)^{\times n}.$$
When $\sSet$ is equipped with the cartesian product and $\cSet$ with the Day convolution product, the triangulation functor is monoidal. Thus it induces a functor $$\mathcal{T} \colon \Mon_{\cSet} \to \Mon_{\sSet}$$ between the associated categories of monoids. 

The triangulation functor has a right adjoint $$\mathcal{U}\colon \sSet \to \cSet$$
