...
\[
Z \to \Big( 2^n \to \Top(\gsimplex^n, Z) \Big)
\]
and referred to as the (simplicial) \textit{singular complex} of the topological space $Z$.
...
We will refer to $\schains(\sSing(Y, y))$ as the \textit{simplicial singular chains on $(Y,y)$} and for simplicity denote it by $\sS(Y,y)$.
