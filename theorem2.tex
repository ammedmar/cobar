...
We now deduce from the above statement a relation between the rigidification functor $\rigid$ and the Kan loop group functor $G$. First we introduce some notation. 
...
\begin{proof}
	...
	By \cref{hinich} we have a natural weak homotopy equivalence $\psi_{G(X)} \colon \classifying(G(X)) \xrightarrow{\simeq} \nerve(G(X))$ for any reduced simplicial set $X$.
	By \cref{p:kan adjuntion}, the unit of the adjunction gives rise to a natural weak homotopy equivalence $X \xrightarrow{\simeq} \classifying(G(X))$.
	Composing these two maps we obtain a natural weak homotopy equivalence
	$$X \xrightarrow{\simeq} \nerve(G(X)).$$
