...
\anibal{Could this be expanded to be more like the intro to the previous section?}
...
\anibal{This and the next section require work}
...
The extended cobar construction of \cite{hess2010cobar} is not functorial on $\coAlg$ since it depends on a choice of basis of the degree one $R$-module of the underlying coalgebra.
...
Denote by
\begin{equation*}
\localization \colon \Mon_{\sSet} \to \sGrp
\end{equation*}
...
If $M$ is an object in $\Mon_{\sSet}$ and $A \in M_0$ is a subset of $0$-morphisms, we denote by $\localization_A M$ the simplicial monoid obtained by formally inverting the elements of $A$, i.e., the push-out
\begin{equation*}
\localization_A M = M \coprod_{A} \localization F(A),
\end{equation*}
...
Finally, denote the composition $\widehat{\ccobar} = \mathcal{P}_{!} \circ \ncobarE$ by
\begin{equation*}
\widehat{\ccobar} \colon \sSet^0 \to \Mon_{\cSet}
\end{equation*}
which is referred to as the \textit{extended cubical cobar construction}.

As an immediate consequence of \cref{l:ccobar and cobar}, we obtain the following isomorphism after localizing. 

\anibal{Maybe also cite something from the previous subsection}
...
Further details regarding the weak equivalence $\localization \rigid(X) \xra{\simeq} G(X)$, including an explicit description in terms of the Szczarba operators, may be found in \cite{minichello2021path}.
