...
The \textit{triangulation} functor $\triangulate \colon \cSet \to \sSet$ is defined by
\[
...
If $\cSet$ is equipped with the Day convolution monoidal structure (\cref{ss:day convolution}), then the functors $\cchains$ and its lift $\cchainsA$ are symmetric monoidal.
...
M(1,2) = \k[\S_2] \left\{ \; \coproduct \ \right\},
\]
\[
...
We remark that this Hopf algebra structure is obtained by extending the assignments:
\begin{align*}
