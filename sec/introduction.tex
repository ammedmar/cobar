...
Returning to based loop spaces, one may obtain an algebraic model for them by considering their singular chains.

\anibal{Discuss E-infty bialgebra on cubical singular chains on loops before}
...
\begin{theorem} \label{t:1st main thm in the intro}
	...
	\end{tikzcd}
	\]
	Furthermore, for any pointed space $(Z,z)$, Adams' comparison map $\theta_Z \colon \cobar \sSchainsAS(Z, z) \to S^{\cube}(\loops_z Z)$ is a quasi-isomorphism of $E_{\infty}$-bialgebras or, more specifically, of monoids in $\coAlg_\UM$.
\end{theorem}
