...
A form of the \textit{homotopy hypothesis}, a cornerstone of modern algebraic topology, recasts the homotopy theory of pointed spaces in terms of topological groups.
...
We will focus on spaces up to weak homotopy equivalence, where a first example of this algebraization process is provided by the functor of singular chains
\[
\Schains \colon \Top \to \Ch,
\]
which associates a chain complex over a fixed commutative ring to any topological space. 

The singular chains construction may be thought of as a way of linearizing a topological space. More homotopical information can be encoded if the resulting chain complex is equipped with non-linear algebraic structure approximating the diagonal map $Z \to Z \times Z$. In other words, if we regard the singular chains as a differential graded (dg) coassociative coalgebra, or, better still, as a homotopy coherent coassociative and cocommutative coalgebra, also known as an $E_\infty$-coalgebra.
...
One may obtain an algebraic model for based loop spaces by considering their singular chains.
A smaller model was introduced by F.~Adams in 1956 via the \textit{cobar construction} \cite{adams1956cobar}, a functor
\[
\cobar \colon \coAlg^\ast \to \Mon_{\Ch}
\]
from the category of coaugemented dg coassociative coalgebras to the category of dg associative algebras, which we regard in this article as monoids in $\Ch$.
He constructed, for any pointed space $(Z,z)$, a \textit{comparison map}
\begin{equation} \label{e:adams map}
\theta_Z \colon \cobar \sSchainsAS(Z,z) \to \cSchains(\loops_z Z),
\end{equation}
a natural chain map of monoids in $\Ch$, where $\sSchainsAS(Z,z)$ is the coalgebra of simplicial singular chains on $(Z,z)$ and $\cSchains(\loops_z Z)$ is the monoid of cubical singular chains on its based loop space.
...
In this article we enhance the cobar construction $\cobar \sSchainsAS(Z,z)$ by explicitly constructing an $E_\infty$-coalgebra structure compatible with its monoid structure, and show that Adams' comparison map is a quasi-isomorphism of monoids in the category of $E_\infty$-coalgebras.
...
With the Day convolution monoidal structure the singular complex functor $\cSing \colon \cSet \to \Top$ and the functor of (normalized) chains $\cchains \colon \cSet \to \Ch$ are monoidal.

Cubical chains are equipped with a natural coalgebra structure coming from Serre's chain approximation to the diagonal.
This structure lifts $\cchains$ to the category of coalgebras as a monoidal functor $\cchainsA \colon \cSet \to \coAlg$, which, in particular, makes $\cSchains(\loops_z Z) = \cchains \cSing(\loops_z Z) $ into a monoid in $\coAlg$ for any pointed space $(Z, z)$.
...
\end{equation}
remarking that there are two key differences between this construction and Baues'.
First, he worked in the context of $1$-reduced simplicial sets, whereas we consider reduced simplicial sets to treat the case of non-simply connected spaces.
Second, we equip cubical sets with the data of connections in order to obtain, before geometric realization, a functorial construction.

Adams' comparison map factors as a composition
\begin{equation} \label{e:baues factorization of adams map}
\begin{tikzcd}[column sep = small]
\theta_Z \colon \cobar \sSchainsAS(Z, z) \arrow[r] &
...
Baues' idea is to transfer the Serre coproduct on $\cchainsA(\ccobar X)$ to $\cobar \schainsA(X)$, making it into a bialgebra, that is to say, a monoid in $\coAlg$.
...
\end{tikzcd}
\]
Furthermore, when $(Z, z)$ is a pointed space, Adams' comparison map $\theta_Z \colon \cobar \sSchainsAS(Z, z) \to S^{\cube}(\loops_z Z)$ is a quasi-isomorphism of bialgebras, i.e., of monoids in $\coAlg$.

Our first result extends Baues' work by lifting $\cobar \schainsA$ to the category of $E_\infty$-bialgebras, that is of monoids in the category of coalgebras equipped with the additional structure of an infinite family of coherent homotopies enforcing, in a derived sense, the coassociativity and cocommutativity of the coproduct.
The study of $E_\infty$-structures has a long history, where (co)homology operations \cite{steenrod1962cohomology, may1970general}, the recognition of infinite loop spaces \cite{boardman1973homotopy, may1972geometry}, and algebraic models for homotopy types \cite{mandell2001padic} are key milestones.

In more detail, our first result constructs a lift of $\cchainsA \ccobar \colon \sSet^0 \to \Mon_{\coAlg}$ to the category of monoids in $\coAlg_\UM$, where $\coAlg_\UM$ is the category of coalgebas over the operad $\UM$ introduced via a finitely presented prop in \cite{medina2020prop1}.
...
Once we have shown that $\coAlg_\UM$ is a monoidal category, we prove the main technical result of this work, stating that the lift $\cchainsUM \colon \cSet \to \coAlg_\UM$ of $\cchainsA$ defined in \cite{medina2021cubical} is monoidal.
...
In particular, for any pointed space $(Z, z)$ Adams' comparison map $\theta_Z \colon \cobar \sSchainsAS(Z, z) \to S^{\cube}(\loops_z Z)$ is a quasi-isomorphism of $E_{\infty}$-bialgebras.
...
\begin{theorem} \label{t:1st main thm in the intro}
	...
	\end{tikzcd}
	\]
	Furthermore, when $(Z, z)$ is a pointed space, Adams' comparison map $\theta_Z \colon \cobar \sSchainsAS(Z, z) \to S^{\cube}(\loops_z Z)$ is a quasi-isomorphism of $E_{\infty}$-bialgebras or, more specifically, of monoids in $\coAlg_\UM$.
\end{theorem}
