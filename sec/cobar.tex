...
%associating to any space $\fX$ the space
%functor
%\[
%P \colon \Top \to \Top
%\]
...
\anibal{Please explain the monoid structure on $\cSchains(\loops_x \fX)$ here.}
...
\begin{enumerate}
	...
	\item $\theta_n \circ \delta_0^j = P(\delta^j) \circ \theta_{n-1}$,
...
\[
\begin{split}
	\delta^{f^n_j} \defeq \delta^n \dotsm \, \delta^{n-j+1} &\,\colon\ \gsimplex^j \to \gsimplex^n, \\
	\delta^{\ell^n_{n-j}} \defeq \delta^{n-j-1} \dotsm \, \delta^{0} &\, \colon\ \gsimplex^{n-j} \to \gsimplex^n,
\end{split}
\]
...
Adams showed the existence of a (non-unique) collection of such maps $\{\theta_n\}$ by induction, using the contractibility of $P(\gsimplex^n;v_0,v_n)$.
One then defines $\theta_{\fX}$ as follows.
