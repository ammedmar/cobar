...
We remark that Day convolution can be defined for general presheaves over a monoidal category. Namely, if $(\mathcal{J}, \boxtimes)$ is a monoidal category, we may define a monoidal structure
\[ 
\otimes \colon \Fun[\mathcal{J}^{\op}, \Set] \times \Fun[\mathcal{J}^{\op}, \Set] \to \Fun[\mathcal{J}^{\op}, \Set] 
\]
by the formula
\[
F \otimes G =\colim_{\substack{\mathcal{Y}(i) \to F \\ \mathcal{Y}(j) \to G}} \mathcal{Y}(i \boxtimes j),
\]
where $\mathcal{Y} \colon \mathcal{J} \to \Fun[\mathcal{J}^{\op}, \Set]$ denotes the Yoneda embedding
$j \mapsto \mathcal{J}(-,j).$
