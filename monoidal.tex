...
The Cartesian product of simplicial sets coincides with the more general notion of the categorical product, since limits of presheaves are computed object-wise. 

Given cubical sets with connections $Y, Y'$ their \textit{Day convolution} is defined by
\begin{equation*}
Y \otimes Y' = \colim_{\substack{\cube^n \to Y \\ \cube^{m} \to Y}} \square^{n+m}.
\end{equation*}
This notion makes $\cSet$ into a (non-symmetric) monoidal category with unit $\cube^0$.
The Day convolution monoidal structure can be defined more generally for presheaves over a monoidal category.
